        ...
        <StaggerContainer className="grid grid-cols-2 lg:grid-cols-4 gap-6" staggerDelay={0.1}>
           {[
             ...
             <StaggerChild key={i} direction="up">
               <div className="bg-white p-8 rounded-[2rem] border border-slate-100 text-center shadow-lg shadow-slate-100/30 hover:border-primary/20 transition-all group">
              ...
              {collegeAchievements.map((item, index) => (
                <ScrollReveal key={index} direction={index % 2 === 0 ? 'left' : 'right'} delay={index * 0.08}>
                  <motion.div 
                    ...
                    className="bg-white p-10 rounded-[2.5rem] border border-slate-50 shadow-lg flex flex-col items-start group hover:shadow-xl transition-all h-full"
                  >
