                ...
                <div className="space-y-12">
                  ...
                  <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" staggerDelay={0.08}>
                    {allWorkshops.map((w, i) => {
                      const dirs = ['left', 'up', 'right', 'up', 'zoom', 'up'] as const;
                      const dir = dirs[i % dirs.length];
                      return (
                        <StaggerChild key={w.id || i} direction={dir}>
                          <div
                            onClick={() => setSelectedImage({ url: w.photo, title: w.title, category: w.category })}
                            className="group bg-white rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-500 cursor-pointer flex flex-col h-full"
                          >
                ...
                <div className="space-y-16">
                  ...
                  <StaggerContainer className="grid grid-cols-2 lg:grid-cols-4 gap-6" staggerDelay={0.1}>
                    {aeroFeatures.map((f, i) => {
                      const Icon = f.icon;
                      return (
                        <StaggerChild key={i} direction={i % 2 === 0 ? 'left' : 'right'}>
                          <div
                            className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:shadow-md hover:-translate-y-1 transition-all duration-300"
                          >
