  ...
  return (
    <PageLayout title="Diploma in Engineering" subtitle="SCTE & VT Affiliated | Industry-Ready Skills | 3-Year Programs">
      ...
      <div className="flex flex-col gap-10 mt-4">
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 items-stretch" staggerDelay={0.09}>
           {courses.map((course, i) => {
             const dirs = ['left', 'up', 'right', 'up', 'zoom'] as const;
             const dir = dirs[i % dirs.length];
             return (
               <StaggerChild key={i} direction={dir}>
                 <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col gap-4 hover:shadow-lg hover:-translate-y-1 transition-all relative group h-full">
